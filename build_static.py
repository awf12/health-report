...
function extractValues(fullText) {
  ...
  const patterns = {
    ...
    'proton pressure': /Proton pressure\\s+(\\d+)/i,
    'electron pressure': /Electron pressure\\s+(\\d+)/i,
    'major resonant frequency of the client': /Major resonant frequency[^\\d]*(\\d+)/i,
    'reactance speed index': /Reactance speed index\\s+(\\d+)/i,
    'phase angle': /Phase angle\\s+(\\d+)/i,
  ...
  // Catch-all: match any \"EnglishLabel Number\" pair
  // Format 1: \"Label (description) VALUE 0 %\"
  const re1 = /([A-Za-z][A-Za-z0-9\\s,().&+\\-:\\/]{2,80}?)\\s+(\\d{2,4})\\s+\\d+\\s*%/g;
  let m;
  while ((m = re1.exec(fullText)) !== null) {
    const name = m[1].trim().toLowerCase();
    const val = parseInt(m[2]);
    if (val >= 0 && val <= 30000 && name.length >= 2) valueMap[name] = val;
  }
  // Format 2: \"LABEL DIGITS\" (risk scores, simple names)
  const re2 = /([A-Z][A-Z0-9\\s\\-]{1,30})\\s+(\\d{1,5})\\b/g;
  while ((m = re2.exec(fullText)) !== null) {
    const name = m[1].trim().toLowerCase();
    const val = parseInt(m[2]);
    if (val >= 0 && val <= 30000 && !(name in valueMap)) valueMap[name] = val;
  }
  // Format 3: Broader catch for any English label followed by a number
  const re3 = /\\b([A-Za-z][A-Za-z0-9\\s,().&+\\-:\\/]{3,60}?)\\s+(\\d{1,5})\\b/g;
  while ((m = re3.exec(fullText)) !== null) {
    const name = m[1].trim().toLowerCase();
    const val = parseInt(m[2]);
    if (val >= 0 && val <= 30000 && name.length >= 2 && !(name in valueMap) && !/^\\d+\\s*\\/\\s*\\d+$/.test(name)) {
      valueMap[name] = val;
    }
  }
  // Fallback: search for body fat anywhere in text
  if (valueMap['body fat'] === undefined) {
    const bfMatch = fullText.match(/body\\W*fat\\W*(\\d+)/i);
    if (bfMatch) { valueMap['body fat'] = parseInt(bfMatch[1]); console.log('✅ body fat fallback:', bfMatch[1]); }
  }
  console.log('Extracted values:', Object.keys(valueMap).length);
  if (valueMap['body fat'] !== undefined) console.log('✅ body fat captured:', valueMap['body fat']);
  else {
    console.log('❌ body fat NOT in valueMap');
    // Print first 500 chars to see what's around 'fat'
    var fatIdx = fullText.toLowerCase().indexOf('fat');
    if (fatIdx >= 0) console.log('First fat context:', fullText.substring(Math.max(0,fatIdx-30), fatIdx+50));
    else console.log('NO fat found in fullText at all!');
  }
  return valueMap;
}
...
function lookupValue(valueMap) {
  ...
  for (const name of names) {
    ...
    const key_underscore = key.replace(/\\s+/g, '_');
    if (valueMap[key_underscore] !== undefined) return valueMap[key_underscore];
...
function processTemplate(valueMap, meta) {
  ...
  let updated = 0;
  for (const [sectionKey, sections] of Object.entries(data.sections)) {
    if (!Array.isArray(sections)) continue;
    for (const sec of sections) {
      const items = sec.indicators || sec;  // Handle both flat and nested structures
      ...
      for (const item of items) {
        ...
        if (v !== null && v !== oldVal) {
          item.value = v;
          updated++;
          if (item.name && item.name.indexOf('体脂') >= 0) console.log('✅ 体脂率 updated:', oldVal, '→', v);
        } else if (item.name && item.name.indexOf('体脂') >= 0) {
          console.log('❌ 体脂率 NOT updated: oldVal=' + oldVal + ' lookup=' + v + ' cn=' + item.cn);
        }
      }
    }
  }
...
async function processAll() {
  ...
  for (let i = 0; i < pendingFiles.length; i++) {
    ...
    try {
      ...
      if (Object.keys(valueMap).length < 10) {
        console.warn('⚠️ 提取值太少，PDF可能格式不兼容');
      }
...
function downloadReport(name) {
  ...
  function cellStyle(v) {
    if (v == null) return {};
    if (v <= 50) return {fill:{fgColor:{rgb:'FFE0E0'}}};
    if (v >= 100) return {fill:{fgColor:{rgb:'FFF8E1'}}};
    return {};
  }
  ...
  for (const key of sheetOrder) {
    ...
    for (const sec of sections) {
      ...
      const title = (sec.title||key).substring(0,31).replace(/[\/:*?"<>|]/g, '-');
      if (key === '5.脊柱反应性') {
        const rows = [['脊柱名称','压力状态','对应身体部位']];
        for (const item of items) {
          const parts = (item.desc||'').split('|');
          rows.push([item.name||'', (parts[1]||'').trim(), item.food||'']);
        }
        const ws = XLSX.utils.aoa_to_sheet(rows);
        ws['!cols'] = [{wch:10},{wch:15},{wch:60}];
        XLSX.utils.book_append_sheet(wb, ws, title);
      } else if (key === '16.神经递质') {
        const rows = [['检测项目','反应值','检测项目','反应值']];
        for (let ni = 0; ni < items.length; ni += 2) {
          const a = items[ni]||{}, b = items[ni+1]||{};
          rows.push([a.name||'', a.value!=null?a.value:'', b.name||'', b.value!=null?b.value:'']);
        }
        const ws = XLSX.utils.aoa_to_sheet(rows);
        ws['!cols'] = [{wch:25},{wch:8},{wch:25},{wch:8}];
        XLSX.utils.book_append_sheet(wb, ws, title);
      } else {
        const headers = (key.includes('基本体质')||key.includes('压力指数')) ? ['检测项目','反应值','数值说明','身体反应与症状'] :
          (key.includes('氨基酸')||key.includes('矿物质')||key.includes('水溶性')||key.includes('脂溶性')||key.includes('碳水')||key.includes('蛋白质')) ? ['检测项目','反应值','详解','食物来源'] :
          ['检测项目','反应值','详解'];
        const rows = [headers];
        for (const item of items) {
          const row = [item.name||'', item.value!=null?item.value:'', item.desc||''];
          if (headers.length >= 4) row.push(item.food||'');
          rows.push(row);
        }
        const ws = XLSX.utils.aoa_to_sheet(rows);
        for (let r = 1; r <= items.length; r++) {
          const cr = XLSX.utils.encode_cell({r:r, c:1});
          if (ws[cr]) ws[cr].s = cellStyle(items[r-1].value);
        }
        ws['!cols'] = headers.map(function(h,i){ return {wch: i===0?20:i===1?10:i===2?40:40}; });
        XLSX.utils.book_append_sheet(wb, ws, title);
      }
    }
  }
...
function refreshList(page) {
  ...
  list.innerHTML = pageNames.map(function(n){
    ...
    return '<div class="customer-card"><div class="info"><strong>'+n+'</strong><span>'+(c.date||'')+' · '+(c.filename||'')+'</span></div>' +
      ...
      '<button class="btn-sm btn-dl" onclick="downloadPDF(\\''+n+'\\')">📄 PDF</button>' +
