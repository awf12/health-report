...
function downloadReport(name) {
  ...
  coverWs['!cols'] = [{wch:12},{wch:20},{wch:12},{wch:25}];
  ...
  for (const key of Object.keys(data.sections)) {
    ...
    for (const sec of sections) {
      ...
      for (const item of items) {
        rows.push([item.name||'', item.value!=null?item.value:'', item.desc||'', item.food||'']);
      }
      const ws = XLSX.utils.aoa_to_sheet(rows);
      // Apply color to value cells
      for (let r = 1; r <= items.length; r++) {
        const cellRef = XLSX.utils.encode_cell({r:r, c:1});
        if (ws[cellRef]) {
          ws[cellRef].s = cellStyle(items[r-1].value);
        }
      }
      ws['!cols'] = [{wch:20},{wch:10},{wch:40},{wch:40}];
