...
function downloadReport(name) {
  ...
  chartInitJS += `_cfgs.forEach(function(cfg){
    ...
    if(!cv) return;
    var vals=cfg.values;
    var colors=vals.map(function(v){return v<=50?'rgba(192,57,43,0.85)':v>=100?'rgba(212,160,23,0.85)':'rgba(65,105,225,0.85)';});
    var borders=vals.map(function(v){return v<=50?'rgba(192,57,43,1)':v>=100?'rgba(212,160,23,1)':'rgba(65,105,225,1)';});
    new Chart(cv,{type:"bar",data:{labels:cfg.labels,datasets:[{data:vals,backgroundColor:colors,borderColor:borders,borderWidth:1}]},
    ...
    plugins:[{id:"bl_"+cfg.chartId,afterDraw:function(ch){
      ...
      m.data.forEach(function(b,i){var v=vals[i],x=b.x,y=b.y-4;
