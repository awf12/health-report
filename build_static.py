...
function downloadReport(name) {
  ...
  // Capture all canvas charts as images
  const canvases = document.querySelectorAll('canvas');
  const chartImages = {};
  canvases.forEach(function(cv) {
    if (cv.id) {
      try { chartImages[cv.id] = cv.toDataURL('image/png'); } catch(e) {}
    }
  });

  // Build HTML with embedded chart images
  let html = '<html><head><meta charset="UTF-8"><style>table{border-collapse:collapse;margin:10px 0;}td,th{border:1px solid #ccc;padding:4px 8px;font-size:12px;}th{background:#f0f0f0;}h3{color:#2c5f2d;}</style></head><body>';

  // Cover
  html += '<h1>营养与健康检测报告</h1>';
  html += '<table><tr><td>姓名:</td><td>'+(M.name||'')+'</td><td>性别:</td><td>'+(M.gender||'')+'</td></tr>';
  html += '<tr><td>出生日期:</td><td>'+(M.birthDate||'')+'</td><td>检测日期:</td><td>'+(M.testDate||'')+'</td></tr>';
  html += '<tr><td>就诊编号:</td><td>'+(M.visitNumber||'')+'</td><td>检测师:</td><td>'+(M.practitioner||'')+'</td></tr></table><br>';

  // Data sections with chart images
  for (const key of Object.keys(data.sections)) {
    ...
    for (let si = 0; si < sections.length; si++) {
      ...
      if (chartImages[chartId]) {
        html += '<img src="' + chartImages[chartId] + '" style="max-width:700px;"><br>';
      }
      // Data table
