...
function renderFullReport(data) {
    ...
    let html = `
<div style="max-width:100%;margin:0;padding-left:200px;" id="cover">
    ...
    for (let i = 0; i < sectionKeys.length; i++) {
        ...
        for (let si = 0; si < sections.length; si++) {
            ...
            if (key.includes('脊柱')) {
                ...
                // All items are already deduped. Sort by value for low/high
                const withVals = items.filter(function(i){ return i.value != null; });
                const sortedAsc = [].concat(withVals).sort(function(a,b){ return (a.value||0) - (b.value||0); });
                const sortedDesc = [].concat(withVals).sort(function(a,b){ return (b.value||0) - (a.value||0); });
                const low10 = sortedAsc.slice(0, 10);
                const high10 = sortedDesc.slice(0, 10);
                html += `<div class="report-card" id="${id}"><h3>${title}</h3>`;
                const emoLabels = {name:'检测项目',value:'反应值'};
                if (low10.length) {
                    html += '<h4>📉 反应值最低的10项</h4>';
                    html += `<div class="chart-wrap"><canvas id="chart_emo_low"></canvas></div>`;
                    html += makeDataTable(low10, ['name','value'], emoLabels);
                    makeBarChart('chart_emo_low', low10);
                }
                if (high10.length) {
                    html += '<h4>📈 反应值最高的10项</h4>';
                    html += `<div class="chart-wrap"><canvas id="chart_emo_high"></canvas></div>`;
                    html += makeDataTable(high10, ['name','value'], emoLabels);
                    makeBarChart('chart_emo_high', high10);
                }
                if (withVals.length) {
                    html += '<h4>📊 全部情绪特征</h4>';
                    html += `<div class="chart-wrap"><canvas id="chart_emotions"></canvas></div>`;
                    html += makeDataTable(withVals, ['name','value'], emoLabels);
                }
                html += '</div>';
                makeBarChart('chart_emotions', withVals);
            } else {
                // 列配置完全按模板
