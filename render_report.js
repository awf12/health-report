    ...
    for (let i = 0; i < sectionKeys.length; i++) {
        ...
        for (let si = 0; si < sections.length; si++) {
            ...
            if (key.includes('脊柱')) {
                ...
                // All items after low10 are the full emotion list (already deduped in template)
                const allEmotions = items.slice(10);
                const sorted = [].concat(allEmotions).sort(function(a,b){ return (b.value||0) - (a.value||0); });
                ...
                if (allEmotions.length) {
                    ...
                    html += makeDataTable(allEmotions, ['name','value'], emoLabels);
                }
                html += '</div>';
                makeBarChart('chart_emotions', allEmotions);
            } else {
                // 列配置完全按模板
